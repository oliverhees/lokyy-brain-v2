...
- **LLM endpoint host allow-list** (LBV2-19): in guarded mode (`VAULT_PROXY_SECRET` or `VAULT_REQUIRE_PROXY_SECRET` set), or whenever `VAULT_LLM_ALLOWED_HOSTS` is set, the configured LLM/embeddings endpoint must use a listed host (exact, case-insensitive; e.g. `api.eurouter.ai`, base URL `https://api.eurouter.ai/api/v1`). `PUT /api/config` (when provider or `baseUrl` changes) and `POST /api/config/test` answer `400 LLM endpoint not allowed`; the adapters (OpenAI/DeepSeek, Anthropic, Ollama) and semantic search in web server and MCP refuse non-listed hosts on every request; redirects are only followed within the same origin. **Guarded deployments without `VAULT_LLM_ALLOWED_HOSTS` make no LLM calls** (fail closed, logged at startup) — set it for the web server and the MCP process. Provider defaults (e.g. `api.openai.com`, Ollama `localhost`) must be listed too. Local setups without either variable are unchanged.
