...
- Coolify template for one company server (LBV2-16): `deploy/coolify/compose.yml` (inner Traefik behind coolify-proxy, same isolation model as `deploy/stack`, vault slots u1–u3 + company vault, memory limits from LBV2-20), Authentik beta blueprint without demo users, `gen-env.sh`, and `docs/beta-runbook.md` (setup, users, provisioning, EUrouter, MCP clients, smoke tests, host egress rules, rotation, backup, rollback, open staging checks).
...
- **Shared embedding service** (LBV2-26, `deploy/stack/embed`, Docker target `embed`): one BGE-M3 instance per server instead of one per vault. Vaults with `MINDBASE_EMBED_URL` + `MINDBASE_EMBED_TOKEN` never load the model (web server indexing and hybrid search, MCP `semantic_search`); per-vault tokens (`EMBED_TOKEN_SHA256_<VAULT>`, or plain `EMBED_TOKEN_<VAULT>` hashed at startup; only hashes kept), optional source-network binding (`EMBED_SOURCE_<VAULT>`), request/text/body limits, 2048-token cap per text (also in-process) and per-request token budget, per-vault rate limit, fair round-robin queue with search queries first, inference timeout with restart (HTTP 500 is not retried; the indexer skips such a page until it changes), bulk slot guarantee (`EMBED_BULK_EVERY`), static errors, logs without text. Vectors match the in-process embedder. See `docs/self-hosting-mcp-http.md#shared-embedding-service`.
- EUrouter routes (LBV2-30): EUrouter is configured by route only. `ruleId` (UUID, stable on rename) and `ruleName` in the config; requests to `api.eurouter.ai` send `rule_id` and no `model` (web server and MCP). Settings has an EUrouter provider with a Route picker and no Model field; the composer and status bar show the route name. `GET`/`POST /api/config/eurouter/rules` list the key's enabled routing rules (id, name, model; admin-only in guarded mode). The EUrouter connection test checks key and route via `/routing-rules` (before, any key passed because `/models` is public) and then sends a minimal rule-only chat. Wrong key and unknown/disabled route get plain messages. PDF chat through EUrouter sends locally extracted text via chat/completions (never `/responses`), limited by `maxContextChars`. `POST /api/config/test` and `/api/config/eurouter/rules` are rate limited (`VAULT_CONFIG_PROBE_RATE`, `VAULT_CONFIG_PROBE_WINDOW_MS`; one window shared by all admins). Local PDF extraction everywhere is bounded: `VAULT_PDF_MAX_BYTES` (25 MB, before parsing), `VAULT_PDF_MAX_PAGES` (500), `VAULT_PDF_TIMEOUT_MS` (60 s); PDF chat stops extracting once the text passes `maxContextChars`.
...
- **LLM endpoint host allow-list** (LBV2-19): in guarded mode (`VAULT_PROXY_SECRET` or `VAULT_REQUIRE_PROXY_SECRET` set), or whenever `VAULT_LLM_ALLOWED_HOSTS` is set, the configured LLM/embeddings endpoint must use a listed host (exact, case-insensitive; `host` allows only https on 443, plain http needs `host:80`, other ports need `host:port`, e.g. `ollama:11434`; e.g. `api.eurouter.ai`, base URL `https://api.eurouter.ai/api/v1`). `PUT /api/config` (when provider or `baseUrl` changes) and `POST /api/config/test` answer `400 LLM endpoint not allowed`; the adapters (OpenAI/DeepSeek, Anthropic, Ollama) semantic search in web server and MCP, and capture transcription (Whisper) refuse non-listed hosts or ports on every request; the Ollama onboarding routes (`/api/system`, `/api/ollama/status`, `/api/ollama/pull`) return 404 in guarded mode, `/api/health` reports `features.localModels: false` and the setup wizard disables the local-model option; redirects are only followed within the same origin. **Guarded deployments without `VAULT_LLM_ALLOWED_HOSTS` make no LLM calls** (fail closed, logged at startup by the web and MCP HTTP servers) — set it for the web server and the MCP process. Provider defaults (e.g. `api.openai.com`, Ollama `localhost`) must be listed too. Local setups without either variable are unchanged.
...
- **Switching to a keyless provider (Ollama) without a key clears the stored cloud API key** (the chat model switch now sends only `provider` and `model`); enter the cloud key again when switching back. Since LBV2-14 a `PUT /api/config` that switches back to a cloud provider with the masked key but no stored key returns 400 (`enter the API key`) instead of saving the provider without a key; the same applies to any switch to a provider that needs a key when the key is omitted or empty. A new secret that contains `********` is rejected with 400. The web UI shows the server's error message for failed saves and connection tests.
...
- **Contributor usernames are ASCII only** (LBV2-14): `[A-Za-z0-9_.-]`, not starting with `.` or `-`, at most 64 characters, no `..`, and `unknown` (any case) is reserved everywhere, also for `X-Mindbase-User` without the proxy guard. Names with non-ASCII letters (e.g. `jürgen`) that were accepted before now get 400; existing contributor directories with such names are no longer reachable through the API. The OS username fallback maps non-ASCII characters to `_` (`jürgen` → `j_rgen`) and uses `user` when nothing ASCII remains. MCP `mindbase_contribute` applies the same rules: an explicit `user` is validated strictly, over stdio an omitted `user` is the sanitized OS account, and over HTTP `user` is now required.
- OCR (LBV2-14): tesseract.js language data is cached in `$MINDBASE_MODEL_CACHE/tesseract` (absolute, writable), else `/models/tesseract`, else the OS temp dir, instead of the read-only working directory.
- MCP `quick_capture` (LBV2-14) returns the data-dir-relative path (`inbox/<timestamp>-capture.md`) instead of the absolute host path.
- MCP (LBV2-14): invalid `MCP_HTTP_READONLY_LLM_*` values no longer stop stdio servers; the HTTP server still validates them and exits 1 at startup.
...
- LBV2-32: the ingest plan step retries once on no tool calls, HTTP 429/5xx, timeouts and network errors (`MINDBASE_COMPILE_RETRIES`, max 3, `MINDBASE_COMPILE_RETRY_DELAY_MS`; one `[compile] retry n/m <reason>` log line per retry); the ingest dialog has a **Retry** button, an accessible close button and readable labels for every planned action. Compile errors shown to web users are fixed texts; provider response bodies go to the server log only (audit L1).
- LBV2-32: sources ingested via `/api/ingest/text` (and other `/api/ingest/*` routes) are listed by `/api/tree/raw` under their raw id with title and open in the raw source view, also on the v1 data layout a fresh data dir gets (before: "Failed to load"; v1 answered 409).
- LBV2-32: MCP `search_wiki`, `ask_wiki`, `semantic_search`, `read_wiki_page` and wiki resources now cover `wiki/concepts` (where compile writes), not only `wiki/notes`; pages the web server writes are indexed without an MCP restart (`MINDBASE_MCP_INDEX_SYNC_MS`, default 2 s). Reader visibility applies to concepts as to notes.
