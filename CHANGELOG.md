...
- Coolify packages S (15 vault slots) and M (30) for one company server (LBV2-27, replaces the LBV2-16 template): generated by `deploy/coolify/generate.ts` (`compose-s.yml`, `compose-m.yml`, inner Traefik routes, Authentik blueprints). Git-based Coolify application, Docker Compose build pack in Raw mode; operator input only `BASE_DOMAIN` + `ADMIN_EMAIL`, all secrets are Coolify magic variables, first start fully automatic (Authentik admin + blueprint, MetaMCP init, model prefetch). Same isolation model as `deploy/stack`; inner Traefik uses a baked-in file provider (no Docker socket). `docs/beta-runbook.md` is now a short "Deploy in Coolify" guide.
