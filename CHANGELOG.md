...
- **Local stack (LBV2-26)**: vaults no longer mount `/models`; they embed through the new `embed` service on per-vault `embed-<vault>` networks and have a memory limit (`VAULT_MEM_LIMIT`, default `1g`; the service `EMBED_MEM_LIMIT`, default `3584m`; measurements in `deploy/stack/README.md`). `.env` needs `EMBED_TOKEN_<VAULT>` plus `./embed-tokens.sh`; `rotate-secrets.sh` rotates them. `STACK_HTTP_PORT`, `STACK_NET` and `IMAGE_TAG` allow a second stack next to a running one.
