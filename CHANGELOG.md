...
- LBV2-32: the ingest plan step retries once on no tool calls, HTTP 429/5xx, timeouts and network errors (`MINDBASE_COMPILE_RETRIES`, `MINDBASE_COMPILE_RETRY_DELAY_MS`); the ingest dialog has a **Retry** button. Compile errors shown to web users are fixed texts; provider response bodies go to the server log only (audit L1).
