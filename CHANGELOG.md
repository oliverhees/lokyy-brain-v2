...
- EUrouter routes (LBV2-30): `ruleId` (UUID) in the config, sent as `rule_id` next to `model` to `api.eurouter.ai` only (web server and MCP); with a route the model is optional. Settings has an EUrouter provider with a Route picker. `GET`/`POST /api/config/eurouter/rules` list the key's routing rules (id, name, model; admin-only in guarded mode). The connection test for EUrouter now checks the key via `/routing-rules` — before, any key passed because EUrouter's `/models` is public — and reports a configured route that does not exist.
