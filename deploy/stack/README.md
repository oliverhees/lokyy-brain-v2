...
`tests/embed.sh` (23 checks): service vectors are identical to the former in-process vectors (4 texts incl. umlauts, emoji and one over 8,000 chars: cosine 1.0000000, max |Δ| 0), and vault-anna stays below 400 MiB (`VAULT_RSS_LIMIT_MIB`) while 200 pages are indexed through the service and 20 hybrid searches run; `embed` stays below its limit and is not OOM-killed; worst-case token-heavy input (section 3) is refused above the token budget, stays inside the limit (cgroup `memory.peak`) and does not block a concurrent search query for more than `QUERY_MAX_S` (15 s); section 4 checks that a note created via MCP is found by the web app's hybrid search and a web note by MCP `semantic_search` (one project-scoped store). It writes and afterwards deletes 200 test pages in vault-anna. All suites first run `tests/port-gate.sh` (no hard-coded host port).
