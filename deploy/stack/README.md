...
| MCP clients (AI tools) | One MetaMCP endpoint per user, API key only (no OAuth, no key in the query string). Traefik routes only `/metamcp/<endpoint>/mcp`, and only to `mcp-gate`, which binds each MCP session to the API key (SHA-256) and endpoint that initialized it and forwards nothing but POST/GET/DELETE on that path to MetaMCP. The gate rejects requests without a key before reading the body (1 MB limit, 30 s request timeout, connection cap), creates sessions only for `initialize` requests, allows at most 20 open sessions per key (a key over the cap loses its own oldest) and refuses new keys when its global table is full instead of evicting anyone. Traefik only reads labels of this compose project, and the MetaMCP admin router never serves `/metamcp/*`. Traefik rate-limits it per client IP (20 req/s, burst 60) and replaces every MetaMCP error response (400–599 except 429) with one static body, turning `403`/`404` into `401`: missing or wrong key, another user's endpoint, an unknown endpoint and an unknown session look identical, and MetaMCP's `available_sessions` list never reaches a client |
