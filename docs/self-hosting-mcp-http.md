...
**LLM config reload (LBV2-32).** The MCP process re-reads `<dataDir>/mindbase.config.json` when it changes (web Settings, setup portal, `configure-eurouter.sh`); no restart is needed. It checks the file's modification time and size at most once per `MINDBASE_MCP_CONFIG_RELOAD_MS` (default `1000`; a missing or invalid value uses the default, `0` checks on every access). A file that is not valid JSON keeps the last good config and is logged once per version (path only, never the content); a deleted file means "LLM not configured". Read-only sessions follow the change too and still see only provider and model.

**Compiled pages in MCP search (LBV2-32).** `search_wiki`, `ask_wiki`, `semantic_search` (including its keyword fallback), `read_wiki_page`, and the `mindbase://wiki/<slug>` resources cover both root-wiki layers: `wiki/notes` and `wiki/concepts`, where ingest (compile) writes. A slug that exists in both layers is read from `wiki/notes` (`semantic_search` prefers `wiki/concepts`, like the embedding indexer). Pages the web server writes while the MCP server runs become searchable without a restart: before each tool or resource request the server compares the page files' modification times and sizes, at most once per `MINDBASE_MCP_INDEX_SYNC_MS` (default `2000`; a missing or invalid value uses the default, `0` checks on every request), and rebuilds the keyword index if something changed. **Cost:** each check lists both layers and stats every page file (two per page); a change triggers a full rebuild that reads every page body, so a busy wiki with thousands of pages that the web server edits continuously costs up to one full rebuild per interval. For large wikis raise `MINDBASE_MCP_INDEX_SYNC_MS` (e.g. `30000`); new pages then become searchable in MCP with that delay. Read-only sessions filter concept pages by visibility like notes: `internal`/`pii` concepts stay invisible.
...
EUrouter is configured **by route only**. A route is an EUrouter routing rule: it has a display name (may contain spaces) and a UUID, and it decides which models and providers answer. The vault stores the UUID (stable when the rule is renamed) and never sends a model id with it; EUrouter answers `POST /chat/completions` with only `rule_id` (verified with a real key).

- **Config.** `ruleId` (UUID) and `ruleName` (display only, max 255 characters) in `mindbase.config.json`. `ruleId` is kept only while `baseUrl` is `https://api.eurouter.ai/...` (https only); saving another endpoint drops both. A value that is not a UUID is refused (`400 Invalid EUrouter rule id`). Switching to EUrouter, or clearing the route of an EUrouter config, without a route is refused (`400 Select an EUrouter route`). An older EUrouter config without a route (e.g. written by `configure-eurouter.sh`) still sends its `model` and stays savable for unrelated settings. Changing only the route never asks for the API key again; changing `baseUrl` still does.
- **Requests.** With a route, chat requests to `api.eurouter.ai` carry `rule_id` and no `model`. `rule_id` is never sent to any other host. The web server and the MCP process both read `ruleId`. Chat errors the user can fix are translated: `401/403` → `EUrouter key invalid or not authorised`; an unknown or disabled route → `Route not found or disabled — choose a route again in Settings`. Embeddings do not use EUrouter.
- **PDF chat.** EUrouter's `/responses` endpoint only reaches a few providers (none behind an EU rule), so PDFs are never sent there: the vault extracts the text locally (pdfjs) and sends it via `chat/completions` with `rule_id`. Text longer than `maxContextChars` (default 50000 characters) is refused before any provider call with `The PDF text (N characters) is too long for the model context (limit M characters)…`; a PDF without extractable text (e.g. scanned) is refused too. Figures and layout are lost on this path. Other OpenAI-compatible hosts keep using `/responses`.
- **PDF extraction limits.** Every local PDF extraction (PDF chat, upload ingest, URL ingest, raw-tree import, Google Drive import, MCP `mindbase_ingest_file`) goes through one bounded extractor: `VAULT_PDF_MAX_BYTES` (default 26214400 = 25 MB, checked before parsing), `VAULT_PDF_MAX_PAGES` (default 500), `VAULT_PDF_TIMEOUT_MS` (default 60000, whole extraction). For PDF chat the extraction also stops as soon as the text passes `maxContextChars`. A limit hit fails with a clear message (e.g. `Could not extract text from the PDF: PDF has more than 500 pages`) and nothing is sent to the provider. Invalid values make extraction fail instead of lifting the limit. Set them for the web server and the MCP process. Limitation: pdfjs runs with its fake worker on the main thread, so the timeout is checked between pages; a single very expensive page (or the initial document parse) runs to completion before the check. The byte and page limits bound how much work that can be.
- **Connection test.** For `api.eurouter.ai`, `POST /api/config/test` needs a route, checks key and route with `GET /api/v1/routing-rules` (EUrouter's `/models` is public and accepts any key), then sends a minimal chat with only `rule_id` (`max_tokens: 16`). HTTP 200 counts as connected even with empty text, because reasoning models may spend those tokens on reasoning. Messages passed to the UI: `EUrouter key invalid or not authorised`, `EUrouter routing rule not found or disabled`, `Select an EUrouter route`, `Route not found or disabled — choose a route again in Settings`; everything else is the generic `Connection test failed` (details are logged without the key).
- **Tool calls (LBV2-32).** Ingest (compile) only works with models that return tool calls. After the checks above, the connection test sends one tiny rule-only request with a single tool (`max_tokens: 256`). If the route answers in text only, the test still passes but returns `{ ok: true, warning: "Connected, but the selected route's model doesn't support tool calls needed for ingest — choose a route with tool-capable models." }`; Settings shows the warning and saves only on **Save anyway**, onboarding shows it on the result step. A failed probe is logged and ignored. At ingest time, a first compile turn with text only (the prompt asks for a short takeaways narrative before the first tool call, and some models stop there) gets one follow-up asking for the tool calls; if the model still returns none, the ingest fails with a message saying so (before, the ingest finished and wrote nothing).
- **Retries and error texts (LBV2-32).** The ingest plan step is repeated once, after a backoff, when it ends without tool calls (routes that spread over several providers answer inconsistently; seen live), on HTTP 429/5xx, on timeouts and on network errors: `MINDBASE_COMPILE_RETRIES` (default `1`, `0` = off, at most `3`: larger values are clamped to 3 and an invalid value uses the default, both with a warning in the server log) and `MINDBASE_COMPILE_RETRY_DELAY_MS` (default `2000`, doubled for each further retry). Other 4xx are not retried. Each retry logs one line, e.g. `[compile] retry 1/1 no tool calls` or `[compile] retry 1/2 HTTP 503` (reason only, no provider body). The ingest dialog offers **Retry** in its error state. Compile errors reach web users only as fixed texts (rejected key, rate limit, request rejected, provider unavailable, no tool calls, route unavailable, timeout, or `Compile failed. Details are in the server log.`); the provider's response body is logged on the server (`[compile] …`) and never sent to the browser, in `/api/compile/:rawId` (502), the plan stream and the ingest streams.
- **Output cap for compile (LBV2-32).** Compile requests (plan and direct compile) send `max_tokens` (as `max_completion_tokens`) of 4096 per call; override with `MINDBASE_COMPILE_MAX_TOKENS` (positive integer). Without it EUrouter rejected compile requests with `estimated tokens exceed context`.
- **Route list for the UI.** `GET /api/config/eurouter/rules` lists the rules for the stored key; `POST /api/config/eurouter/rules` with `{ provider, baseUrl, apiKey }` does the same for a key typed into the form (the masked key resolves to the stored key only for the stored provider and endpoint, as in `/api/config/test`). Both return only `{ rules: [{ id, name, model }] }` (`model` = the rule's first model, shown as a hint), are **admin-only in guarded mode (GET included)** because they use the stored key, answer `400` when the endpoint is not EUrouter, `400 EUrouter key invalid or not authorised` when EUrouter answers 401/403, and a generic `502 Could not load EUrouter routes` on other upstream errors. EUrouter returns all rules in one unpaginated `{ data }`; disabled rules (`enabled: false`) are left out.
- **Rate limit.** `POST /api/config/test` and `POST /api/config/eurouter/rules` share one sliding window per server: `VAULT_CONFIG_PROBE_RATE` requests (default 20) per `VAULT_CONFIG_PROBE_WINDOW_MS` (default 60000). Over the limit: `429` with `Retry-After`. Invalid values stop the server at startup instead of disabling the limit. The window is **shared by all admins of the vault** (not per user or IP): one admin probing keys at the limit also blocks other admins' tests and route lists until the window frees up. That is intended for a single-team vault; raise `VAULT_CONFIG_PROBE_RATE` if several admins configure at once.
- **Web UI.** Settings → Provider → **EUrouter** has an API key and a Route picker (rule name shown, id stored) with loading, empty and error states, and **no Model field**. The composer shows the route name instead of the model switcher, and the status bar shows the route name.
...
| `config-reload.mjs` | none (library) | LBV2-32: a config written, changed or broken after start is picked up without restart (context and reader view), an invalid file keeps the last good config and is logged without its content, checks are throttled by `MINDBASE_MCP_CONFIG_RELOAD_MS`. |
| `concepts-search.mjs` | none (in-memory MCP) | LBV2-32: a concept page written after start is found by `search_wiki`, cited by `ask_wiki`, ranked by `semantic_search`, readable via `read_wiki_page` and the wiki resource; for readers a public concept is found and cited, an `internal` concept never reaches search results, answers or the LLM prompt. |
