...
- **PDF extraction limits.** Every local PDF extraction (PDF chat, upload ingest, raw-tree import, Google Drive import, MCP `mindbase_ingest_file`) is bounded: `VAULT_PDF_MAX_BYTES` (default 26214400 = 25 MB, checked before parsing), `VAULT_PDF_MAX_PAGES` (default 500), `VAULT_PDF_TIMEOUT_MS` (default 60000, whole extraction). For PDF chat the extraction also stops as soon as the text passes `maxContextChars`. A limit hit fails with a clear message (e.g. `Could not extract text from the PDF: PDF has more than 500 pages`) and nothing is sent to the provider. Invalid values make extraction fail instead of lifting the limit. Set them for the web server and the MCP process.
