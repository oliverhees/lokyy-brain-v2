...
| `MINDBASE_EMBED_RETRIES` | Optional, extra attempts on `408`, `429`, `5xx` and connection errors, default `2` (exponential backoff from 500 ms; `Retry-After` is honoured up to 10 s). Other `4xx` and the client's own timeout are not retried. The client sends at most 4 texts per request. |
