...
- **Tool calls (LBV2-32).** Ingest (compile) only works with models that return tool calls. After the checks above, the connection test sends one tiny rule-only request with a single tool (`max_tokens: 256`). If the route answers in text only, the test still passes but returns `{ ok: true, warning: "Connected, but the selected route's model doesn't support tool calls needed for ingest — choose a route with tool-capable models." }`; Settings shows the warning and saves only on **Save anyway**, onboarding shows it on the result step. A failed probe is logged and ignored. At ingest time, a model that answers the first compile turn without any tool call fails the ingest with a message saying so (before, the ingest finished and wrote nothing).
