...
- **Config.** `ruleId` in `mindbase.config.json` (UUID). It is kept only while `baseUrl` is `https://api.eurouter.ai/...` (https only); saving another endpoint drops it. A value that is not a UUID is refused (`400 Invalid EUrouter rule id`). Changing only the route never asks for the API key again; changing `baseUrl` still does.
- **Requests.** Chat requests to `api.eurouter.ai` carry `rule_id` as a top-level body field next to `model`. `rule_id` is never sent to any other host. The web server and the MCP process both read `ruleId`. Embeddings do not use EUrouter.
- **PDF chat.** EUrouter's `/responses` endpoint only reaches a few providers (none behind an EU rule), so PDFs are never sent there: the vault extracts the text locally (pdfjs) and sends it via `chat/completions` with `model` and `rule_id`. Text longer than `maxContextChars` (default 50000 characters) is refused before any provider call with `The PDF text (N characters) is too long for the model context (limit M characters)…`; a PDF without extractable text (e.g. scanned) is refused too. Figures and layout are lost on this path. Other OpenAI-compatible hosts keep using `/responses`.
...
- **Web UI.** Settings → Provider → **EUrouter** shows a Route picker (rule name shown, id stored) with loading, empty and error states. The Model field stays required: switching from another provider clears it, and picking a route fills in the route's default model (editable).
