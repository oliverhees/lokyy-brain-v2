...
- **Retries and error texts (LBV2-32).** The ingest plan step is repeated once, after a backoff, when it ends without tool calls (the second follow-up-free attempt often lands on another provider of the route), on HTTP 429/5xx, on timeouts and on network errors: `MINDBASE_COMPILE_RETRIES` (default `1`, `0` = off) and `MINDBASE_COMPILE_RETRY_DELAY_MS` (default `2000`, doubled for each further retry). Other 4xx are not retried. The ingest dialog offers **Retry** in its error state. Compile errors reach web users only as fixed texts (rejected key, rate limit, request rejected, provider unavailable, no tool calls, route unavailable, timeout, or `Compile failed. Details are in the server log.`); the provider's response body is logged on the server (`[compile] …`) and never sent to the browser, in `/api/compile/:rawId` (502), the plan stream and the ingest streams.
