...
- **Concurrency.** At most `MINDBASE_FETCH_CONCURRENCY` fetches (default 4, integer 1–64) run at once per process; further requests wait. An invalid value makes the MCP HTTP server exit at startup; in the web server, fetches fail.
