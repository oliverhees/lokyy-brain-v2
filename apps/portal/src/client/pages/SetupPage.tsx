...
function StepFrame({ heading, children, error, footer, onSubmit, fields }:
  { heading: string; children: ReactNode; error: string | null; footer: ReactNode; onSubmit: (e: FormEvent) => void; fields: Record<string, string> }) {
  ...
  // After a validation error the first invalid field gets the focus.
  useEffect(() => {
    if (Object.keys(fields).length > 0) formRef.current?.querySelector<HTMLElement>('[aria-invalid="true"]')?.focus();
  }, [fields]);
  return (
    <form ref={formRef} onSubmit={onSubmit} noValidate aria-labelledby={id} className="flex flex-col gap-5">
...
export function SetupPage({ onDone, initialStep }: { onDone: () => void; initialStep?: number }) {
  ...
  if (current === 0) {
    body = (
      <StepFrame fields={fields} heading={t.company.heading} error={stepError}
    ...
    body = (
      <StepFrame fields={fields} heading={t.llm.heading} error={stepError}
    ...
    body = (
      <StepFrame fields={fields} heading={t.smtp.heading} error={stepError}
    ...
    body = (
      <StepFrame fields={fields} heading={t.done.heading} error={stepError}
