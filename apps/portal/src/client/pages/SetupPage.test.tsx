...
const empty: SetupStatus = {
  company: null, llm: null, smtp: null, setupCompletedAt: null, vaults: ['firma', 'v01', 'v02'], slots: { total: 2, free: 2 }, package: null,
};
const configured = { mode: 'shared' as const, model: null, baseUrl: 'https://api.eurouter.ai/api/v1',
...
describe('SetupPage wizard', () => {
  ...
  it('a rejected key is shown at the key field', async () => {
    ...
    expect(m.container.textContent).toContain('EUrouter lehnt diesen Schlüssel ab.');
  });
  ...
  it('an account without routes gets a clear hint', async () => {
    ...
    await type(input(m.container, 'EUrouter-API-Schlüssel'), KEY);
    await click(byText(m.container, 'Routen laden'));
    expect(m.container.textContent).toContain('noch keine Route');
  });
