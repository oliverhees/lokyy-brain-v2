...
export interface PortalConfig {
  ...
  publicOrigin: string;
  staticDir: string;
  authentik: { url: string; token: string };
...
export function loadConfig(env: Record<string, string | undefined>): PortalConfig {
  ...
  return {
    ...
    proxySecret,
    publicOrigin: env['PORTAL_PUBLIC_ORIGIN'] ?? `https://app.${domain}`,
    staticDir: env['PORTAL_STATIC_DIR'] ?? new URL('../../dist/client', import.meta.url).pathname,
    authentik: { url: env['AUTHENTIK_URL'] ?? 'http://authentik-server:9000', token: req('AUTHENTIK_API_TOKEN') },
    metamcp: {
      url: metamcpUrl,
      databaseUrl: req('METAMCP_DATABASE_URL'),
      publicBase: env['METAMCP_PUBLIC_BASE'] ?? `https://mcp.${domain}`,
