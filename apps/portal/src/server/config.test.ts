...
const base: Record<string, string> = {
  ...
  LOKYY_SLOTS: 'v01,v02',
  PORTAL_PROXY_SECRET: 's'.repeat(40),
...
describe('loadConfig', () => {
  ...
  it.each(['LOKYY_DOMAIN', 'LOKYY_SLOTS', 'PORTAL_PROXY_SECRET', 'AUTHENTIK_API_TOKEN', 'METAMCP_DATABASE_URL', 'VAULT_ADMIN_URL'])('requires %s', (name) => {
...
describe('HttpVaultAdmin', () => {
  it('PUTs the EUrouter config to /<vault>/api/config of the internal admin entrypoint', async () => {
    const calls: { url: string; init: RequestInit }[] = [];
    const va = new HttpVaultAdmin('http://lokyy-traefik:8090/', async (url, init) => {
      calls.push({ url: String(url), init: init! });
      return new Response('{"ok":true}', { status: 200 });
    });
    await va.configureLlm('v01', { apiKey: 'sk-1234567890', model: 'mistral/x' });
    expect(calls[0]!.url).toBe('http://lokyy-traefik:8090/v01/api/config');
    expect(calls[0]!.init.method).toBe('PUT');
    expect(JSON.parse(String(calls[0]!.init.body))).toEqual({ provider: 'openai', baseUrl: 'https://api.eurouter.ai/api/v1', model: 'mistral/x', apiKey: 'sk-1234567890' });
  });
  ...
  it('throws on a refused config without echoing the body', async () => {
    ...
    const err = await va.configureLlm('v01', { apiKey: 'sk-1234567890', model: 'm' }).catch((e: Error) => e);
  ...
  it('refuses vault names that could change the path', async () => {
    ...
    await expect(va.configureLlm('../v01', { apiKey: 'k', model: 'm' })).rejects.toThrow(/vault/);
  });
});
